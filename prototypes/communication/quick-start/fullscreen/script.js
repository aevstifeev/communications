(() => {
  ...
  const quickStartDocumentTitle = document.getElementById("quickStartDocumentTitle");
  ...
  const quickStartFullscreen = document.getElementById("quickStartFullscreen");
  const fullscreenQuickStartCreateButton = document.getElementById("fullscreenQuickStartCreateButton");
  const fullscreenQuickStartJoinInput = document.getElementById("fullscreenQuickStartJoinInput");
  const fullscreenQuickStartJoinButton = document.getElementById("fullscreenQuickStartJoinButton");
  const quickStartRecordingTriggers = document.querySelectorAll("[data-open-quick-start-recording]");
  ...
  if (quickStartFullscreen && boardShell && quickStartFullscreen.parentElement !== boardShell) {
    boardShell.appendChild(quickStartFullscreen);
  }
  ...
  const state = {
    ...
    quickStartFullscreenOpen: false,
    recordingsUnread: false,
    recordingDocumentOpen: false,
    recordingDocumentTocOpen: true,
    quickStartRecordingTitle: "Кайтен ВКС тестовая запись",
  ...
  function openMeeting() {
    state.quickStartFullscreenOpen = false;
  ...
  function openMeetingWindow() {
    const meetingUrl = new URL(window.location.href);
    meetingUrl.searchParams.set("meeting", "1");

    const meetingWindow = window.open(meetingUrl.toString(), "_blank", "noopener,noreferrer,width=1440,height=920");
  ...
  function getRecordingTitle(trigger) {
    return trigger?.dataset.recordingTitle
      || trigger?.querySelector(".record-row-main strong")?.textContent?.trim()
      || "Кайтен ВКС тестовая запись";
  }

  function openQuickStartTranscriptDrawer(title = state.quickStartRecordingTitle) {
    state.quickStartRecordingTitle = title;
    state.recordingDocumentOpen = false;
    state.quickStartFullscreenOpen = true;
    state.taskOpen = true;
  ...
  function render() {
    ...
    if (leftPanelTitle) {
      const chatTitle = communicationTabButtons.length
        ? (communicationTabTitles[state.communicationTab] || leftModeTitles.chat)
        : leftModeTitles.chat;
    ...
    leftModeButtons.forEach((button) => {
      const isActive = button.dataset.leftMode === "chat"
        ? state.quickStartFullscreenOpen
        : state.leftOpen && button.dataset.leftMode === state.leftMode;
    ...
    if (workspace) {
      ...
      workspace.classList.toggle("quick-start-open", state.quickStartFullscreenOpen);
      workspace.classList.toggle("quick-start-document-open", state.quickStartFullscreenOpen && state.taskOpen && state.detailKind === "document");
    }
    ...
    if (workspaceBody) {
      ...
      workspaceBody.classList.toggle("quick-start-open", state.quickStartFullscreenOpen);
      workspaceBody.classList.toggle("quick-start-document-open", state.quickStartFullscreenOpen && state.taskOpen && state.detailKind === "document");
    }
    ...
    if (quickStartFullscreen) {
      quickStartFullscreen.hidden = !state.quickStartFullscreenOpen;
    }
    ...
    if (quickStartDocumentTitle) {
      quickStartDocumentTitle.textContent = state.quickStartRecordingTitle;
    }
  ...
  leftModeButtons.forEach((button) => {
    button.addEventListener("click", () => {
      ...
      if (requestedMode === "chat") {
        state.quickStartFullscreenOpen = !state.quickStartFullscreenOpen;
        state.leftOpen = false;
        state.leftMode = "chat";
        state.recordingDocumentOpen = false;
        state.taskOpen = false;
  ...
  if (quickStartCreateButton) {
    quickStartCreateButton.addEventListener("click", () => {
      openMeetingWindow();
    });
  }

  if (quickStartJoinButton) {
    quickStartJoinButton.addEventListener("click", () => {
      const joinValue = quickStartJoinInput ? quickStartJoinInput.value.trim() : "";

      if (!joinValue) {
        if (quickStartJoinInput) {
          quickStartJoinInput.focus();
        }
        return;
      }
  ...
  quickStartRecordingTriggers.forEach((trigger) => {
    trigger.addEventListener("click", () => {
      openQuickStartTranscriptDrawer(getRecordingTitle(trigger));
    });
  });

  if (fullscreenQuickStartCreateButton) {
    fullscreenQuickStartCreateButton.addEventListener("click", () => {
      openMeetingWindow();
    });
  }

  if (fullscreenQuickStartJoinButton) {
    fullscreenQuickStartJoinButton.addEventListener("click", () => {
      const joinValue = fullscreenQuickStartJoinInput ? fullscreenQuickStartJoinInput.value.trim() : "";

      if (!joinValue) {
        if (fullscreenQuickStartJoinInput) {
          fullscreenQuickStartJoinInput.focus();
        }
        return;
      }
